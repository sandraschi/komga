...
class LlmApiController(
    ...
    fun getProviders(): List<LlmProviderDto> {
        return LlmProvider.values().map { provider ->
            LlmProviderDto(
                id = provider.name,
    ...
    fun getActiveProvider(): LlmProviderDto {
        return llmServiceFactory.getActiveProvider()?.let { provider ->
            LlmProviderDto(
                id = provider.name,
                name = provider.name.replace('_', ' '),
                enabled = true,
    ...
    suspend fun switchProvider(@PathVariable providerId: String): ResponseEntity<LlmProviderDto> {
        return try {
            val provider = LlmProvider.valueOf(providerId.uppercase())
            val service = llmServiceFactory.switchProvider(provider)
            
            ResponseEntity.ok(LlmProviderDto(
                id = provider.name,
                name = provider.name.replace('_', ' '),
                enabled = true,
                isActive = true
    ...
    suspend fun generateCompletion(
        ...
        return try {
            val result = llmService.generateCompletion(
        ...
        return try {
            val messages = request.messages.map { 
                ChatMessage(
                    ...
                    content = it.content,
                    name = it.name,
                    functionCall = it.functionCall?.let { fc ->
                        FunctionCall(
                            name = fc.name,
            ...
            val functions = request.functions?.map {
                FunctionDefinition(
                    name = it.name,
            ...
            val functionCall = request.functionCall?.let {
                FunctionCall(
                    name = it.name,
            ...
            val result = llmService.generateChatCompletion(
                messages = messages,
                maxTokens = request.maxTokens ?: 1000,
                temperature = request.temperature ?: 0.7,
                functions = functions,
            ...
            LlmApiDto.ChatCompletionResponse(
                ...
                functionCall = result.functionCall?.let { fc ->
