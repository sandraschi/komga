...
open class JpaVirtualBookRepository : VirtualBookRepository {
  ...
  override fun saveAll(virtualBooks: Collection<VirtualBook>): java.util.List<VirtualBook> = 
    virtualBooks.map { save(it) }.toList().let { java.util.ArrayList(it) }
  ...
  override fun findByOmnibusIdIn(omnibusIds: Collection<String>): java.util.Map<String, java.util.List<VirtualBook>> {
    ...
    return omnibusIds.associateWith { result[it] ?: java.util.ArrayList() }
