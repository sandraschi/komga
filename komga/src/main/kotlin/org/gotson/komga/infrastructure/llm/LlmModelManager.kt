...
class LlmModelManager(
    ...
    suspend fun loadModel(provider: LlmProvider, modelName: String): String {
        val modelId = "${provider.name.toLowerCase()}-${UUID.randomUUID()}"
        ...
        activeModels[modelId] = ActiveModelInfo(
            modelId = modelName,
            provider = provider
        )
    ...
    private suspend fun listOllamaModels(): List<ModelInfo> {
        return try {
            val response = restTemplate.getForObject<OllamaListResponse>(
    ...
    private suspend fun unloadOllamaModel(modelName: String) {
        try {
            restTemplate.delete("${llmConfig.ollama.apiUrl}/api/delete", 
                mapOf("name" to modelName))
        } catch (e: Exception) {
    ...
    private suspend fun listVllmModels(): List<ModelInfo> {
        return try {
            val response = restTemplate.getForObject<VllmListResponse>(
