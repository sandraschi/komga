...
import org.gotson.komga.infrastructure.llm.model.LlmService
...
abstract class BaseLlmService(
  ...
  protected fun createHeaders(
    ...
    mutableMapOf("Content-Type" to contentType).apply {
   ...
   */
  override suspend fun createEmbedding(input: String): List<Double> = throw UnsupportedOperationException("Embeddings are not supported by ${provider.name}")
