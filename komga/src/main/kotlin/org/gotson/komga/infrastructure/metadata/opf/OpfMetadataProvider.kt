...
class OpfMetadataProvider(
...
) : BookMetadataProvider, SeriesMetadataFromBookProvider, SidecarBookConsumer {
    ...
    override fun getBookMetadataFromBook(book: BookWithMedia): BookMetadataPatch? {
        ...
        val opfFile = File(book.book.path.parent.toFile(), "${book.book.path.nameWithoutExtension}.opf")
        ...
        return try {
            opfFile.reader().use { reader ->
                val opf = Jsoup.parse(reader, null, "", Parser.xmlParser())
    ...
    private fun parseOpfMetadata(opf: org.jsoup.nodes.Document): BookMetadataPatch {
        ...
        val description = opf.selectFirst("metadata > dc|description")?.text()?.let {
