...
/**
 * Implementation of [LlmService] for LM Studio's local API.
 *
 * LM Studio provides a local server that mimics the OpenAI API, making it compatible
 * with existing OpenAI client code while running models locally.
 */
@Service
class LmStudioService(
    private val config: LmStudioConfig,
    private val restTemplate: RestTemplate,
    private val objectMapper: ObjectMapper
) : BaseLlmService(restTemplate) {

    override val provider = LlmProvider.LM_STUDIO
    
    private val logger = KotlinLogging.logger {}
    
    init {
        logger.info { "Initialized LM Studio service with model: ${config.model}" }
    }
    ...
    override suspend fun generateChatCompletion(
        ...
        val functionCallResponse = (message["function_call"] as? Map<*, *>)?.let {
            FunctionCall(
