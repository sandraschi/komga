...
class EpubTocParser(
    ...
    private val titleCleanupRegexes = listOf(
     ...
     */
    fun extractWorks(book: Book, epubFile: File): List<Work> {
        return try {
            ...
            val works = when (detectTocType(toc)) {
    ...
    private fun extractShakespeareWorks(book: Book, epubBook: EpubBook, toc: List<TOCReference>, metadata: Map<String, String>): List<Work> {
    ...
    private fun extractDelphiClassicsWorks(book: Book, epubBook: EpubBook, toc: List<TOCReference>, metadata: Map<String, String>): List<Work> {
    ...
    private fun extractGenericWorks(book: Book, epubBook: EpubBook, toc: List<TOCReference>, metadata: Map<String, String>): List<Work> {
    ...
    private fun fallbackExtraction(book: Book, epubBook: EpubBook, metadata: Map<String, String>): List<Work> {
    ...
    private fun extractMetadata(epubBook: EpubBook): Map<String, String> {
        return buildMap {
            // Basic metadata
            put("title", epubBook.metadata.titles.joinToString("; "))
            
            // Authors
            epubBook.metadata.authors.takeIf { it.isNotEmpty() }?.let { authors ->
                put("authors", authors.joinToString(", ") { "${it.firstname} ${it.lastname}".trim() })
            }
            
            // Language
            epubBook.metadata.language?.let { put("language", it) }
            
            // Publisher
            epubBook.metadata.publishers.takeIf { it.isNotEmpty() }?.let { publishers ->
                put("publisher", publishers.joinToString(", "))
            }
            
            // Description
            epubBook.metadata.descriptions.takeIf { it.isNotEmpty() }?.let { descriptions ->
