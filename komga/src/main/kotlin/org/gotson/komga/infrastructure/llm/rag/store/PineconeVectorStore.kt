...
class PineconeVectorStore(
  ...
  private fun createConnection(): PineconeConnection = client.connect(indexName)
  ...
  override suspend fun similaritySearchWithEmbedding(
    ...
    withContext(Dispatchers.IO) {
      try {
        ...
        // Get full documents
        val documents =
          resultsByDoc.keys.mapNotNull { docId ->
            getDocument(docId)?.let { doc ->
              doc to
                resultsByDoc[docId]!!.map { match ->
  ...
  override suspend fun getDocumentChunks(documentId: String): List<DocumentChunk> = getDocument(documentId)?.chunks ?: emptyList()
