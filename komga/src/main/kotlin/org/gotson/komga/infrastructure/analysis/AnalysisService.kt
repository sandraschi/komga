...
class AnalysisService(
    ...
    suspend fun analyze(books: List<Book>, options: MetaBook.GenerationOptions): BookAnalysis {
        logger.info { "Analyzing ${books.size} books with options: $options" }
        
        // 1. Extract text from books
        val extractedTexts = books.associateWith { book ->
    ...
    private suspend fun analyzeIndividualBook(
        ...
        val response = llmService.generateAnalysis(
            prompt = prompt,
    ...
    private fun buildPrompt(book: Book, text: String, options: MetaBook.GenerationOptions): String {
        return """
