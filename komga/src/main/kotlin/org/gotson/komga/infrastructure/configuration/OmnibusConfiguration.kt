...
class OmnibusConfiguration {
    ...
    @Bean
    fun omnibusDetector(): OmnibusDetector =
        OmnibusDetectorImpl()

    @Bean
    fun epubTocParser(epubContentExtractor: EpubContentExtractor): EpubTocParser =
        EpubTocParser(epubContentExtractor)
        
    @Bean
    fun epubExtractionService(epubContentExtractor: EpubContentExtractor): EpubExtractionService =
        EpubExtractionService(epubContentExtractor)
    ...
    @Bean
    fun virtualBookRepository(jpaVirtualBookRepository: JpaVirtualBookRepository): VirtualBookRepository =
        jpaVirtualBookRepository
    ...
    fun cacheCleanupTask(
        ...
        CacheCleanupTask(virtualBookContentService)
    ...
    fun omnibusController(
        ...
        OmnibusController(virtualBookService, omnibusService)
        
    @Bean
    fun virtualBookContentController(
        virtualBookContentService: VirtualBookContentService,
        virtualBookService: VirtualBookService
    ): VirtualBookContentController =
        VirtualBookContentController(virtualBookContentService, virtualBookService)
        
    @Bean
    fun omnibusTaskController(
        omnibusProcessingTask: OmnibusProcessingTask
    ): OmnibusTaskController =
        OmnibusTaskController(omnibusProcessingTask)
}
