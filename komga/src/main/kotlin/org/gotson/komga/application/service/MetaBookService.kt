...
class MetaBookService(
    ...
    fun getMetaBook(id: String): MetaBook? =
        metaBookRepository.findById(id)

    
    fun getMetaBooksByBookId(bookId: String, pageable: org.springframework.data.domain.Pageable): org.springframework.data.domain.Page<MetaBook> =
        metaBookRepository.findAllByBookId(bookId, pageable)
    
    fun getMetaBooksByUserId(userId: String, pageable: org.springframework.data.domain.Pageable): org.springframework.data.domain.Page<MetaBook> =
        metaBookRepository.findAllByUserId(userId, pageable)
    ...
    fun processMetaBookGeneration(metaBookId: String) {
        ...
        try {
            ...
            val updatedMetaBook = metaBook.copy(
            ...
            // Mark as completed
            val completedMetaBook = updatedMetaBook.copy(
            ...
            val failedMetaBook = metaBook.copy(
    ...
    fun deleteMetaBook(id: String, userId: String) {
        val metaBook = metaBookRepository.findById(id)
    ...
    private fun determineType(bookIds: List<String>, options: MetaBook.GenerationOptions): MetaBook.MetaBookType {
        return when {
    ...
    // Section management
    fun getSections(metaBookId: String): List<MetaBookSection> =
        metaBookRepository.findSectionsByMetaBookId(metaBookId)
        
    fun addSection(metaBookId: String, section: MetaBookSection): MetaBookSection {
        val metaBook = metaBookRepository.findById(metaBookId)
            ?: throw IllegalArgumentException("MetaBook not found: $metaBookId")
    ...
    fun updateSectionProgress(metaBookId: String, sectionType: String, progress: Float) {
