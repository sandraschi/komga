...
export default class MetaBookListView extends Vue {
  ...
  private async loadMetabooks() {
    try {
      ...
      this.$toast.error('Failed to load metabooks')
      console.error(error)
    } finally {
      this.loading = false
    }
  ...
  private viewMetabook(metabook: MetaBook) {
    this.$router.push(`/metabooks/${metabook.id}`)
  }

  private editMetabook(metabook: MetaBook) {
    this.$router.push(`/metabooks/${metabook.id}/edit`)
  }
  ...
  private async deleteMetabook() {
    ...
    try {
      ...
      this.$toast.success('Metabook deleted successfully')
      await this.loadMetabooks()
    } catch (error) {
      this.$toast.error('Failed to delete metabook')
      console.error(error)
    } finally {
