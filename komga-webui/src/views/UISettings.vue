...
export default Vue.extend(({
  ...
  methods: {
    async refreshSettings() {
      ...
      (this as any).form.oauth2HideLogin = (this as any).$store.state.komgaSettings.clientSettingsGlobal[CLIENT_SETTING.WEBUI_OAUTH2_HIDE_LOGIN]?.value === 'true'
      (this as any).form.oauth2AutoLogin = (this as any).$store.state.komgaSettings.clientSettingsGlobal[CLIENT_SETTING.WEBUI_OAUTH2_AUTO_LOGIN]?.value === 'true'
      try {
        (this as any).form.seriesGroups = (JSON.parse((this as any).$store.state.komgaSettings.clientSettingsGlobal[CLIENT_SETTING.WEBUI_SERIES_GROUPS]?.value) as ClientSettingsSeriesGroup)?.name
      } catch (_) {
      }
    ...
    async saveSettings() {
      ...
      if ((this as any).$v.form?.oauth2HideLogin?.$dirty)
        newSettings[CLIENT_SETTING.WEBUI_OAUTH2_HIDE_LOGIN] = {
          value: (this as any).form.oauth2HideLogin ? 'true' : 'false',
          allowUnauthorized: true,
        }

      if ((this as any).$v.form?.oauth2AutoLogin?.$dirty)
        newSettings[CLIENT_SETTING.WEBUI_OAUTH2_AUTO_LOGIN] = {
          value: (this as any).form.oauth2AutoLogin ? 'true' : 'false',
          allowUnauthorized: true,
        }

      if ((this as any).$v.form?.seriesGroups?.$dirty)
        newSettings[CLIENT_SETTING.WEBUI_SERIES_GROUPS] = {
          value: JSON.stringify((this as any).form.seriesGroups === 'alpha' ? SERIES_GROUP_ALPHA : SERIES_GROUP_JAPANESE),
          allowUnauthorized: false,
        }

      await (this as any).$komgaSettings.updateClientSettingGlobal(newSettings)
