...
export default Vue.extend({
  ...
  methods: {
    getLibraryNameById(id: string): string {
      return (this as any).$store.getters.getLibraryById(id).name
    },
    async loadData() {
      (this as any).$komgaMetrics.getMetric('komga.tasks.execution')
        .then(m => {
          this.tasks = m
      ...
      (this as any).$komgaMetrics.getMetric('komga.series')
        .then(m => {
            this.series = m
      ...
      (this as any).$komgaMetrics.getMetric('komga.books')
        .then(m => {
            this.books = m
            this.getStatisticForEachTagValue(m, 'library', 'VALUE', this.getLibraryNameById)
      ...
      (this as any).$komgaMetrics.getMetric('komga.books.filesize')
        .then(m => {
      ...
      (this as any).$komgaMetrics.getMetric('komga.sidecars')
        .then(m => {
            this.sidecars = m
            this.getStatisticForEachTagValue(m, 'library', 'VALUE', this.getLibraryNameById)
      ...
      (this as any).$komgaMetrics.getMetric('komga.collections')
        .then(m => this.collections = m)
        .catch(() => {
        })

      (this as any).$komgaMetrics.getMetric('komga.readlists')
    ...
    async getStatisticForEachTagValue(metric: MetricDto, tag: string, statistic: string = 'VALUE', tagTransform: (t: string) => string = x => x): Promise<{
      ...
      if (tagDto) {
        const tagToStatistic: { [key: string]: number | undefined } = {}
        for (const tagKey of tagDto.values) {
          tagToStatistic[tagTransform(tagKey)] = (await (this as any).$komgaMetrics.getMetric(metric.name, [{
            key: tag,
            value: tagKey,
          }])).measurements.find(x => x.statistic === statistic)?.value
        }
        return (this as any).$_.cloneDeep(tagToStatistic)
      }
      return undefined
    },
  },
