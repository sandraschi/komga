...
export default Vue.extend({
  ...
  methods: {
    async cancelAllTasks() {
      const count = await (this as any).$komgaTasks.deleteAllTasks()
      (this as any).$eventHub.$emit(NOTIFICATION, {
        message: (this as any).$tc('server.server_management.notification_tasks_cancelled', count),
      } as NotificationEvent)
    },
    async stopServer() {
      try {
        await (this as any).$actuator.shutdown()
      } catch (e) {
        (this as any).$eventHub.$emit(ERROR, {message: e.message} as ErrorEvent)
      }
    },
    downloadLogFile() {
      new jsFileDownloader({
        url: `${urls.originNoSlash}${(this as any).$actuator.logfileUrl()}`,
        filename: 'komga.log',
