...
// Weaviate schema for documents
const DOCUMENT_SCHEMA = {
  class: 'Document',
  vectorizer: 'none', // We'll handle embeddings ourselves
  properties: [
    {
      name: 'fileId',
      dataType: ['string'],
      description: 'ID of the file this document belongs to',
    },
    {
      name: 'content',
      dataType: ['text'],
...
class RAGService {
  ...
  async initializeClient() {
    try {
      // Make Weaviate optional - only initialize if URL is provided
      if (!config.vectorDb?.weaviate?.url) {
        logger.warn('Weaviate URL not configured. RAG features will be disabled.');
        this.client = null;
        this.embeddingFunction = null;
        return;
      }

      logger.info('Initializing Weaviate client...', { url: config.vectorDb.weaviate.url });
      
      // Initialize the Weaviate client
      this.client = weaviate.client({
        scheme: 'http',
        host: new URL(config.vectorDb.weaviate.url).hostname,
        port: new URL(config.vectorDb.weaviate.url).port || 80,
      });
      ...
      try {
        ...
        logger.error('Failed to connect to Weaviate server', { error: error.message });
        this.client = null;
        return;
      }
      
      // Initialize OpenAI embeddings (if configured)
      ...
      logger.error('Error initializing RAG service', { error: error.message });
      this.client = null;
      this.embeddingFunction = null;
    }
  }
  ...
  async ensureSchema() {
    ...
    try {
      ...
      const exists = schema.classes.some(cls => cls.class === DOCUMENT_SCHEMA.class);
      
      if (!exists) {
        logger.info('Creating Weaviate schema...');
        await this.client.schema.classCreator().withClass(DOCUMENT_SCHEMA).do();
        logger.info('Weaviate schema created successfully');
      } else {
        logger.debug('Weaviate schema already exists');
      }
    } catch (error) {
      logger.error('Error ensuring Weaviate schema', { error: error.message });
      throw error;
    }
  }
